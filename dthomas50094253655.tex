...
The crux of our experiment is to resolve star trails and detect changes in individual trails. We use four metrics to assess how the exposure time impacts these goals: trail resolution SNR ($S_{res}$), trail change SNR ($S_{change}$), CCD trail-off fraction ($c$), and survey duty cycle ($d$). The trail resolution SNR compares the ratio of flux in the trail to the sky background, source Poisson, and readout noises. Contrary to conventional exposures, this ratio decreases with exposure time because once a trail passes over a pixel it stops providing signal, while the sky background continues to increase with time. Likewise, the trail change SNR compares the immediate change of flux a source to the sky background, source Poisson, and readout noises. The CCD trail ratio is the length of an equatorial star trail over the length of the diagonal of a 2048x4096 pixel DECam CCD. It provides a sense of the fraction of trails that trail off an individual CCD. The duty-cycle is the fraction of exposure time over the combined exposure-readout-slew-time. We use many variables from the \textit{DECam User Guide} in our metrics code, including zero-points, filter transmissions, readout noise, sky brightness, and slew time as a function of angle.

We find that 15 second exposures in the `r' band balances these competing metrics. Under these conditions, for trails with source magnitudes $m < 14$, the resolution is $S_{res} > 10$. We achieve $S_{change} > 5$ for $\Delta m = -0.2, m < 12.5$ or $\Delta m = -1.0, m < 15$. The DDC trail ratio is 0.19, which we interpret as greater than 80\% of the trails are contained within the CCD. The average readout and slew time for our proposed targets is 31 seconds leading to an acceptable duty cycle of $d = 0.33$.

We take three exposures of each target. The first is a static 15 second exposure and serves as a reference image which will be valuable to our testing and analysis. The final two exposures are 15 second trail exposures. Progressing through the four K2 fields, including exposure, slew, and readout time, will take 3 hours and 8 minutes. We would like to keep the remainder of the half-night as a safety buffer. However, if we keep this schedule, then we will have time to do one more pass over the four K2 fields.

There are three periods between May and July that are particularly well suited for our survey. The first half of the night on July 18-31; the second half of the night May 1-13; the second of any night on May28-June5. Have at least 3 hours where each field are at airmass < 1.5 and limited moonlight. Finally, we have confirmed that the listed targets are within the Blanco 4-m/DECam horizon limits.
